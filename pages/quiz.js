...
export async function startMeaning(container, allWords, opts = {}) {
    ...
    function renderQ() {
        ...
        options.forEach((opt) => {
            ...
            btn.style.cssText =
                "text-align:left;font-size:0.9rem;padding:0.75rem;min-height:56px;display:flex;flex-direction:column;gap:4px;align-items:flex-start";
            btn.innerHTML =
                `<span>${esc(opt.text)}</span>` +
                (opt.chinese
                    ? `<span class="opt-chi" style="display:none;font-size:0.85rem;font-weight:600">🇨🇳 ${esc(opt.chinese)}</span>`
                    : "");
        ...
        hintChi?.addEventListener("click", async () => {
            ...
            grid.querySelectorAll(".opt-chi").forEach((el) => {
...
function buildOptions(word, allWords) {
    ...
    while (distractors.length < 3) {
        distractors.push({
            ...
            text: `Definition of ${FALLBACK_WORDS[distractors.length]}`,
            chinese: null,
        });
    }
...
export async function startSpelling(container, allWords, opts = {}) {
    ...
    function renderQ() {
        ...
        function nextEditable(i) {
            let j = i + 1;
            while (j < inputs.length && inputs[j].readOnly) {
                j++;
            }
            return j;
        }
...
function spellingClueHtml(word) {
    ...
    if (word.chinese_definition) {
        parts.push(
            `<p style="font-size:1.1rem;font-weight:600;margin:0 0 0.5rem">🇨🇳 ${esc(word.chinese_definition)}</p>`,
        );
    }
