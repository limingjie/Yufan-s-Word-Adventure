...
export async function render(container) {
    ...
    function redraw() {
        ...
        if (countEl) {
            ...
            countEl.textContent = words.length === total ? `${total} ${noun}` : `${words.length} of ${total} ${noun}`;
        }
    ...
    function renderCalPick() {
        document.getElementById("calPickTitle").textContent = new Date(calPickYear, calPickMonth, 1).toLocaleDateString(
            "en-US",
    ...
    document.addEventListener("click", () => {
        ...
        document.getElementById("calendarPicker")?.style &&
            (document.getElementById("calendarPicker").style.display = "none");
    });
    ...
    function renderTrash() {
        document.getElementById("trashList").innerHTML =
            ...
            <div style="display:flex;align-items:center;gap:0.5rem;padding:0.35rem 0;border-bottom:1px solid #f0f0f0">
                ...
                .join("") || '<p style="color:#aaa;font-size:0.85rem;margin:0.5rem 0">Trash is empty.</p>';
    }
    ...
    function openDrawerForEdit(word) {
        ...
        if (audioUrlUS || audioUrlUK) {
            ...
            if (audioUrlUS) {
                usBtn.textContent = editUsIpa ? `🔊 US ${editUsIpa}` : "🔊 US";
                usBtn.dataset.src = audioUrlUS;
                usBtn.style.display = "inline-flex";
            }
            if (audioUrlUK) {
                ukBtn.textContent = editUkIpa ? `🔊 UK ${editUkIpa}` : "🔊 UK";
                ukBtn.dataset.src = audioUrlUK;
    ...
    function renderFormsChips(forms) {
        ...
        chips.innerHTML = FORMS_ORDER.filter((k) => forms[k])
            .map(
                (k) =>
                    `<span style="background:#f0f4ff;border-radius:3px;padding:2px 6px;font-size:0.78rem;color:#444">${FORMS_LABELS[k]}: <strong>${esc(forms[k])}</strong></span>`,
            )
            .join("");
    ...
    async function drawerLookup() {
        ...
        if (dupWord) {
            const learnedOn = dupWord.created_at ? formatDate(localYMD(new Date(dupWord.created_at))) : null;
        ...
        try {
            ...
            if (dictRes.ok) {
                ...
                const ukIpa =
                    ps.find((p) => p.type === "ipa" && p.tags?.includes("Received Pronunciation"))?.text ?? null;
                ...
                lookupResult = "found";
                // Audio is optional; do not make the form wait for two extra network requests.
                drawerShowPron(word, usIpa, ukIpa, lookupId).catch(() => {});
            } else {
    ...
    async function drawerShowPron(word, usIpa, ukIpa, lookupId) {
        const panel = document.getElementById("drawerPronPanel");
        const usBtn = document.getElementById("drawerPronUS");
        const ukBtn = document.getElementById("drawerPronUK");
        audioUrlUS = null;
    ...
    function drawerFill(data) {
        ...
        if (data.chinese_definition != null)
            document.getElementById("drawerChiDef").value = zhOnly(data.chinese_definition);
    ...
    async function drawerSave() {
        ...
        if (editingWordId === null && lookupResult === "not_found") {
            if (!globalThis.confirm(`"${word}" wasn't found in the dictionary — it may be misspelled. Save anyway?`))
                return;
        }
    ...
    document.getElementById("drawerWordInput").addEventListener("keydown", (e) => {
        if (e.key === "Enter") drawerLookup();
    });
    ...
    document
        .getElementById("drawerPronUK")
        .addEventListener("click", () => playAudio(document.getElementById("drawerPronUK").dataset.src));
    document
        .getElementById("drawerPronUS")
        .addEventListener("click", () => playAudio(document.getElementById("drawerPronUS").dataset.src));
...
async function fetchChineseDefinition(word, sense) {
    ...
    try {
        const zh = await fetchWithTimeout(
...
async function probeAudio(url) {
    try {
        const r = await fetchWithTimeout(url, AUDIO_TIMEOUT_MS);
...
function ipaBodyHtml(w) {
    ...
    const chips = [];
    if (usIpa)
        chips.push(`<span class="pron-chip pron-chip-us" style="opacity:0.88;cursor:default">US ${esc(usIpa)}</span>`);
    if (ukIpa)
        chips.push(`<span class="pron-chip pron-chip-uk" style="opacity:0.88;cursor:default">UK ${esc(ukIpa)}</span>`);
...
function wordCardPronRow(w) {
    ...
    if (w.audio_url_us)
        parts.push(`<button class="play-btn pron-chip pron-chip-us" data-src="${esc(w.audio_url_us)}">🔊 US</button>`);
    if (w.audio_url_uk)
        parts.push(`<button class="play-btn pron-chip pron-chip-uk" data-src="${esc(w.audio_url_uk)}">🔊 UK</button>`);
...
function wordCardBody(w) {
    ...
    const moreSection = extraContent
        ? '<button class="expand-more-btn" data-id="' +
          wid +
          '"' +
          ...
          '<div id="extra-' +
          wid +
          '" style="display:none;margin-top:0.25rem;border-top:1px solid #f0f0f0;padding-top:0.35rem">' +
          extraContent +
          "</div>"
        : "";
...
function formsHtml(forms) {
    ...
    const chips = FORMS_ORDER.filter((k) => forms[k])
        .map(
            (k) =>
                `<span style="background:#f0f4ff;border-radius:3px;padding:1px 5px;font-size:0.75rem;color:#555">${FORMS_LABELS[k]}: <strong>${esc(forms[k])}</strong></span>`,
        )
        .join(" ");
...
function buildPickDay(ds, d, isToday, isSelected, count) {
    ...
    const countHtml =
        count > 0 ? `<div style="font-size:0.6rem;color:${countColor};line-height:1.2">${count}</div>` : "";
