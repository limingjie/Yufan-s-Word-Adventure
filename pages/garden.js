...
export async function render(container) {
    ...
    const placed = items
        .filter(
            (i) =>
                STRUCTURES.includes(i.item_code) ||
                isAnimal(i.item_code) ||
                (isPlaceable(i.item_code) && i.col != null),
        )
        .map((i) => ({
            id: i.id,
            code: i.item_code,
    ...
    if (words.length) {
        ...
        controller = createGarden(canvas, {
            ...
            onAssignHomes: (homes) => {
                if (homes.length) setPlantPositions(homes).catch(() => {});
            },
    ...
    function showPlantPopup(word) {
        ...
        actions.innerHTML =
            '<button id="gWater" class="btn btn-primary btn-sm" style="width:100%">💧 Water it — do you remember?</button>';
        popup.style.display = "block";
    ...
    async function handleGardenReview(word, correct) {
        ...
        const result = await runAfterActivity({
            ...
            answered: 1,
            correct: correct ? 1 : 0,
            perfectSession: correct,
            maxCombo: correct ? 1 : 0,
        });
        ...
        tray.innerHTML = `
            <div class="tray-hint">${
                unplaced.length
                    ...
                    : "Buy roads, rails, fences, runways, vehicles or towers in the 🛒 Shop, then drag them here."
            }</div>
    ...
    function shopCard(it) {
        ...
        const tag =
            it.type === "theme"
                ? ' <span class="shop-tag">theme</span>'
                : it.type === "booster"
                  ? ' <span class="shop-tag">booster</span>'
                  : "";
        return `
    ...
    function renderShop() {
        ...
        const sections = CATS.map(([cat, label]) => {
            const cards = all
                .filter((it) => (it.cat || "decor") === cat)
                .map(shopCard)
                .join("");
            return cards
                ? `<div class="shop-section"><h4 class="shop-cat">${label}</h4><div class="shop-grid">${cards}</div></div>`
                : "";
        }).join("");
    ...
    async function buy(code, requestedQuantity = 1) {
        try {
            ...
            toast(
                err.message === "Not enough coins"
                    ? "🪙 Not enough coins yet — keep learning!"
                    : err.message === "Already owned"
