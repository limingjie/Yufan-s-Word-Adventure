...
10. **Word Garden is a full-screen 3D voxel scene** rendered with Three.js (ESM from CDN), flat colours / Minecraft-style blocks — not a 2D view. The legacy 2D SVG garden is not the product. **Everything is a voxel model built in-code** (flat-colour boxes, no external assets): plants (6 growth stages mapped to mastery, with vivid bloom dots so they pop off the grass), sky critters (flapping wings), ground animals, the gnome, structures, vehicles, fences and signs. **Layout is stored, not derived:** each block has a *surface* (grass/road/rail/**crossing**/**fence**/water/stone) and one *occupant* (plant/vehicle/structure/animal); every plant (`garden_plants`) and placeable or positioned item (`col/grid_row/rotation` on `garden_items`) remembers a fixed `(col,row)`. The field is the content bounding box + a 2-cell padding ring and auto-grows. The **placeable playset** (road/rail/**level-crossing**/**fence**/**station**/car/train) is bought into a tray (shown **at the top** in Arrange mode so it isn't clipped on mobile) and dragged onto blocks; a car needs a road, a train a rail, a station an adjacent rail; roads/rails/fences auto-connect, and **vehicles are voxel models that drive** their connected network with sound — turning to face travel, arcing through corners, random turns at junctions, re-routing on edits, frozen while arranging (driving position is runtime-only). The **one-surface-per-block rule still holds** — road/rail/fence never share a cell; **level crossings are a dedicated `crossing` tile** that belongs to *both* networks (cars drive road+crossing, trains rail+crossing) with two boom gates that lower across the road when a train is near. Cars **keep right**, **queue** behind same-direction cars, **slow for curves**, **stop at Canada-style 3-lamp traffic lights** (red/amber/green; lights only on 4-way junctions) and at **stop signs** (3-way junctions), and hold for a train at a crossing. **Trains dwell at stations.** Structures (pond/fountain/cottage — all **unlimited**, stack with persisted positions) and the gnome's cottage have night-glow windows. Ground animals (including bear and deer) have persisted home cells, can be moved in Arrange mode, and roam only inside the connected non-fence region containing their home. The Garden Shop is a **card grid grouped by category**. See [docs/garden.md](docs/garden.md).
