...
10. **Word Garden is a full-screen 3D voxel scene** rendered with Three.js (ESM from CDN), flat colours / Minecraft-style blocks — not a 2D view. The legacy 2D SVG garden is not the product. **Layout is stored, not derived:** each block has a *surface* (grass/road/rail/water/stone) and one *occupant* (plant/vehicle/structure); every plant (`garden_plants`) and placeable item (`col/grid_row/rotation` on `garden_items`) remembers a fixed `(col,row)`. The field is the content bounding box + a 2-cell padding ring and auto-grows. The **placeable playset** (road/rail/car/train) is bought into a tray and dragged onto blocks in **Arrange mode**; a car needs a road under it and a train a rail, roads/rails auto-connect, and **vehicles are voxel 3D models that drive** their connected network with sound — turning to face travel and arcing through corners, random turns at junctions, re-route when track changes, frozen while arranging (driving position is runtime-only). Cars **keep right**, **stop at auto-placed timed traffic lights** on ≥3-arm junctions, **queue** behind same-direction cars, and **slow for curves**. Structures (incl. the cottage, now a **voxel house** with night-glow windows) are voxel models with persisted positions too. Models are flat-colour boxes built in-code (no external assets). Level crossings are impossible in the one-surface-per-block grid (road/rail never share a cell); train stations and voxelising plants/critters are still pending. See [docs/garden.md](docs/garden.md).
